...
### Fetch text
text = open("/Users/Ami/Desktop/homer_ai/homer_full_text.txt").read()
...
def build_model(vocab_size, embedding_dim, rnn_units, batch_size):
    model = tf.keras.Sequential([
        ...
        tf.keras.layers.Dense(vocab_size) ###新改的地方
    ])
    return model
...
START_WORDS = [u"Book I", u"Book II", u"Book III", u"Book IV", u"Book V", u"Book VI", u"Book VII", u"Book VIII", u"Book IX"]
...
for word in START_WORDS:
    filename = "outputs/{0}_test_out.txt".format(word[:-2])
    print(word, filename)
    out_text = open(filename, "w+")
    out_text.write(generate_text(model, start_string = word))
    out_text.close()
    ...
    out_text = open(filename, "w")
    out_text.write(generate_text(model, start_string = 'test'))
    out_text.close()

    , u"Book III", u"Book IV", u"Book V"
    '''
